...
export default function InvestigationUploadZone() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      const res = await fetch("/api/upload-investigation", { method: "POST", body: formData });
      ...
      >
        ...
        {status === "uploading" ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="font-mono text-xs text-[#8A8A8E]">matching {fileName} against incidents…</p>
          </div>
        ) : (
          <>
            <p className="text-sm font-medium text-[#EDEDEC]">
              Drop the Monthly Fire Incident Monitoring file here
            </p>
            <p className="mt-1 font-mono text-xs text-[#6A6A6E]">.xlsm · .xlsx — click to browse</p>
          </>
        )}
      </div>
        ...
        <div className="border border-t-0 border-[#2A2A2C] bg-[#141415] px-4 py-3 font-mono text-xs">
          {status === "success" ? (
            <div className="flex flex-col gap-1.5">
              ...
              <Row label="TOTAL RECORDS" value={String(result.totalRecords ?? 0)} />
              ...
              <Row label="MATCHED" value={`${result.matchedCount ?? 0} linked to existing incidents`} accent="#3EBD6B" />
              {typeof result.unmatchedCount === "number" && result.unmatchedCount > 0 && (
                <Row
                  label="UNMATCHED"
                  value={`${result.unmatchedCount} need manual review`}
                  accent="#F5A623"
                />
              )}
            </div>
...
function Row({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-baseline gap-3">
      <span className="w-32 shrink-0 text-[#5A5A5E]">{label}</span>
